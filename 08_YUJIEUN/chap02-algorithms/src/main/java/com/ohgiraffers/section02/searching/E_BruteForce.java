...
/*
* 완전탐색(BruteForce) + 백트래킹(Back Tracking)
* 주어진 정수들의 모든 순열(순서 있는 조합) 을 구하는 문제
* 가증한 모든 경우의 수를  탐색하는 '완전 탐색' 방식을 사용하지만,
* 백트래킹을 활용하여 이미 사용한 정수를 다시 선택하지 않음으로써 가지치기를 수행하고
* 불필요한 탐색을 줄여 효윻적으로 모든 순열의 갯수를 구한다.
* */
public class E_BruteForce {
    ...
    /* n : 전체 정수 개수*/
    private static void backtrack(int depth, int n, ArrayList<Object> perm) {
        if(depth == n){
            /* 모든 정수를 한 번씩 사용한 것이므로 유호한 순열 1개를 찾았다는 의미*/
