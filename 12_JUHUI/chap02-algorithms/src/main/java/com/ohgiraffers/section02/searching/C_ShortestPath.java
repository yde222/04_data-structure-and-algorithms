    ...
    public static int solution(String input) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < ch.length; j++) {
                map[i][j] = Character.getNumericValue(ch[j]); //순회로 찾는데 문자 '1'을 숫자 1로 바꾸는 메소드
            }
        }
...
//        }
    ...
    static void bfs(int x, int y) {
        visit[x][y] = true;
        q.add(new Node(x, y)); //q에다가 현재값 넣어줌

        /* 시작 지점에서부터 큐를 이용하여 큐에 담기는 노드가 제거 될 때 까지
        * 너비 우선 탐색을 진행한다. */
        while (!q.isEmpty()) {

            /* 현재 탐색을 진행할 노드 */
            Node node = q.poll();
            for (int i = 0; i < 4; i++) { // 상하좌우 바라보는 거
                ...
                visit[cx][cy] = true;
                map[cx][cy] = map[node.x][node.y] + 1; // 한 칸씩 옮긴다.
