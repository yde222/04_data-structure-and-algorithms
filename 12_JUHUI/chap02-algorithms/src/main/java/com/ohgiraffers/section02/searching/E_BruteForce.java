...
public class E_BruteForce {
    ...
    public static Integer solution(String input) {
        ...
        int n = st.countTokens(); //StringTokenizer가 가지고 있는 메소드가 .countTokens()
        nums = new int[n];
        for(int i = 0; i < n; i++) {
            nums[i] = Integer.parseInt(st.nextToken()); // 어떻게 쪼갰는지 확인
        }
    ...
    /* n : 전체 정수 개수 */
    // perm : 어떤 값을 담을건지
    private static void backtrack(int dept, int n, ArrayList<Integer> perm) {
        ...
        * */
        for (int i = 0; i < n; i++) {
          if (!visited[i]) { // 이미 선택한 정수는 건너뛰어 가지치기 한다.
              visited[i] = true;
              perm.add(nums[i]); // 실제로 정수값을 순열에 추가
              backtrack(dept + 1, n, perm);
              perm.remove(perm.size() - 1); // 백트래킹 : 선택했던 정수의 사용 상태를 원상복구 및 순열에서 제거
              visited[i] = false; // 해당 경우의 수 다시 체크
          }
        }
    }
}
