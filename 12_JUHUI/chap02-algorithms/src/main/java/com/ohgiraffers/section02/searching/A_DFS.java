 ...
 * 시작 노드에서 출발해 한 방향으로 갈 수 있을 만큼 깊이 탐색한 후,
...
public class A_DFS {
    ...
    static int count = 0; // 방문횟수 카운트 변수
    ...
    public static Integer solution(String input) throws IOException {
        ...
        // 받은걸 쪼개기 위한 작업 시작
        node = Integer.parseInt(br.readLine()); // 7이라는 값이 들어갈 거임
        edge = Integer.parseInt(br.readLine()); // 6

        // node를 (x,y)좌표로 만들어야함
        /* map은 그래프의 간선 연결 정보를 이차원 배열로 표현한 내용임 */
        map = new int[node+1][node + 1]; // +1을 하는 이유는 0번 인덱스를 제외하고 사용하기 위함
    ...
    private static void dfsStack(int start) {
        ...
        visit[start] = true; // visit에 활성화시켜줌. 방문배열에 기록

        while(!stack.isEmpty()) {
            int current = stack.pop(); // 더이상 사용하지 않는 거니까 삭제
