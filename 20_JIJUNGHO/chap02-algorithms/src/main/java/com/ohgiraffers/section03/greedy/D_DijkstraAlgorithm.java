...
public class D_DijkstraAlgorithm {
    ...
    public static String solution(String input) throws IOException {
        ...
        // 각 노드의 가중치를 기록할 배열 생성
        dis = new int[n + 1];

        // 아직 거리가 판단되지 않은 경우에는 Integer 최대 값으로 채워둔다.
        Arrays.fill(dis, Integer.MAX_VALUE);    // Arrays.fill() : 값을 채워줌
        ...
        // 우선순위 큐에 Edge가 담겼을때 우선순위는 가중치가 낮은 순서로 정해진다.
        ...
        while (!pq.isEmpty()) {
            ...
            // 기준 정점과 연결된 이웃 정점을 큐에 추가하는 반복문 생성
            for (Edge edge : graph.get(now)) {
                // 순회를 돌면서 거리를 기록해두는 배열에 저장된 값이
