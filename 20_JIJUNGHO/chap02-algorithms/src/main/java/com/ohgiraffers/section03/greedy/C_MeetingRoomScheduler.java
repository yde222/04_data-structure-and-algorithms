...
public class C_MeetingRoomScheduler {
    ...
    public static int solution(String input) throws IOException {
        ...
        int N = Integer.parseInt(br.readLine()); // 회의의 총 갯수
        int[][] time = new int[N][2]; // 회의 갯수 * (시작시간 / 종료시간)
        StringTokenizer st;

        for (int i = 0; i < N; i++) {
            st = new StringTokenizer(br.readLine()); // 시작 시간과 종료시간의 쌍
        ...
        Arrays.sort(time, (int[] t1, int[] t2) -> {
            if (t1[1] == t2[1]) return t1[0] - t2[0]; // 종료시간이 같을때는 시작시간으로 정렬
            return t1[1] - t2[1];
        });

        int endTime = 0;    // 직전 회의가 끝난 시간을 담아둘 변수
        int count = 0;      // 회의가 배정된 회의실 갯수

        /* time 배열 안에 있는 회의를 반복하며 회의 시간표에 넣을지 확인 */
        for (int i = 0; i < N; i++) {
            if (time[i][0] >= endTime) {
                count++;
                endTime = time[i][1];   // 이후 회의 확인을 위해 종료시간을 수정
            }
        }
