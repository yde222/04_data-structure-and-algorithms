 ...
 * 더 이상 진행 할 수 없을 때 이전 분기점으로 돌아가 다른 경로를 탐색하는 알고리즘
 * */
...
public class A_DFS {
    ...
    static int count = 0;       // 웜 바이러스에 걸린 컴퓨터 수
    ...
    private static void dfsStack(int start) {
        ...
        stack.push(start); // stack 값을 stack에 처음 넣어준다.
    ...
    private static void dfsRecusive(int start) {
        ...
        for (int i = 1; i <= node; i++) {
            ...
            if (map[start][i] == 1 && !visit[i]) {
                // 바이러스를 전파할 이웃 노드(컴퓨터)를 찾은 것이므로
                // count를 증가시키고 해당 이웃 노드를 방문해서 다시 이웃 노드를
                // 재귀적으로 탐색한다.
                count++;
                dfsRecusive(i);
            }
